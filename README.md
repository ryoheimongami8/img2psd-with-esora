...
画像生成は社内 Esora API（既定 `gemini_nanobanana_2` / Nanobanana 2）。
...
| 項目 | 既定 | 変えるとき |
|---|---|---|
| 生成モデル（最初の画面） | Nanobanana 2 | 絵柄が好みでないとき。GPT Image 2 は構図が数％ずれるが自動で位置合わせされる |
...
| `*_layers.psd` | 最終PSD（下記5レイヤー） |

PSD のレイヤー構成（下から順）:

| レイヤー | 中身 |
|---|---|
| `source` | アップロードした画像そのもの（不透明） |
| `original` | 同じ画像から背景を抜いたもの |
| `generated` | AI着色（元画像の座標系に位置合わせ済み） |
| `lineart` | 抽出した線画 |
| `mask` | 使用したマスク（グレースケール、既定で非表示） |

`mask` は参照用なので非表示で書き出す。Photoshop の目のアイコンで表示できる。
無圧縮PSD（`compression=0`）なので、2048px の縦長で 50MB 前後になる。
